...
var express  = require('express');								// express webserver for Node
...
var Pusher = require('pusher');										// Pusher for websockets handling
var pusher = new Pusher({													// Initialize Pusher
	appId: process.env.PUSHER_APP_ID,								// Pusher App ID stored in environment variable
	key: process.env.PUSHER_KEY,										// Pusher Key stored in environment variable
	secret: process.env.PUSHER_SECRET								// Pusher Secret stored in environment variable
});
...
app.listen(3000, function() {
	console.log("App listening on port 3000");	// Start node on port 3000
});

require('./app/backend/routes')(router, pusher);	// export router and Pusher
